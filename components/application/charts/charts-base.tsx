    ...
    <ul
      className={cx(
        ...
        layout === 'vertical'
          ...
          : `flex-row flex-wrap gap-x-4 gap-y-2 ${align === 'center' ? 'justify-center' : align === 'right' ? 'justify-end' : 'justify-start'}`,
        className
      )}
    >
      {payload?.map((entry, index) => {
        const hasExplicitColor = entry.color && entry.color !== 'currentColor';
        return (
          <li className="text-tertiary flex items-center gap-2 text-sm" key={index}>
            <span
              className={cx(
                'h-2 w-2 shrink-0 rounded-full',
                !hasExplicitColor && cx('bg-current', (entry.payload as { className?: string })?.className)
              )}
              style={hasExplicitColor ? { backgroundColor: entry.color } : undefined}
            />
            {entry.value}
          </li>
        );
...
export const ChartTooltipContent = ({
  ...
  return (
    <div className="bg-primary-solid flex flex-col gap-0.5 rounded-lg px-3 py-2 shadow-lg">
      {isRadialChart && isSingleDataPoint ? (
        ...
        <div>
          {secondaryTitle.map((entry, index) => (
            <div key={index} className="flex items-center gap-1.5">
              ...
              />
              <p className={cx('text-tooltip-supporting-text text-xs')}>
