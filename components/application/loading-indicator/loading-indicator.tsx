...
type LoadingIndicatorColor = 'brand' | 'error' | 'warning' | 'success' | 'neutral';
...
const STROKE_COLOR_CLASS: Record<LoadingIndicatorColor, string> = {
  brand: 'stroke-fg-brand-primary',
  error: 'stroke-fg-error-primary',
  warning: 'stroke-fg-warning-primary',
  success: 'stroke-fg-success-primary',
  neutral: 'stroke-fg-quaternary',
};

const TEXT_FG_COLOR_CLASS: Record<LoadingIndicatorColor, string> = {
...
export const LoadingIndicator = ({
  ...
  const renderSpinner = () => {
    if (type === 'line-spinner') {
      return (
        <svg className={cx('animate-spin', styles[size].spinner)} viewBox="0 0 32 32" fill="none">
          <circle
            className={STROKE_COLOR_CLASS[color]}
            cx="16"
            cy="16"
            r="14"
            fill="none"
    ...
    if (type === 'dot-circle') {
      return (
        <svg
          className={cx(TEXT_FG_COLOR_CLASS[color], 'animate-spin', styles[size].spinner)}
          viewBox="0 0 36 36"
          fill="none"
        >
          <path
