...
export const statuses = {
  incomplete: {
    icon: 'opacity-50 ring-[1.5px] ring-inset ',
    ...
    >
      <span
        className={cx(
          'rounded-full',
          statuses[status].dot,
          size === 'xs' ? 'size-1' : size === 'sm' ? 'size-2' : 'size-2.5'
        )}
      />
      ...
      {status === 'complete' && (
        <svg
          ...
          viewBox="0 0 12 12"
          fill="none"
          className={cx(size === 'xs' ? 'size-2' : size === 'sm' ? 'size-3' : 'size-4')}
        >
          <path
            d="M10 3L4.5 8.5L2 6"
            stroke="white"
      ...
      >
        <p
          className={cx(
            ...
            size === 'xs' || size === 'sm' ? 'text-sm font-semibold' : 'text-md font-semibold'
          )}
        >
          {title}
        </p>
        <p
          className={cx(
            ...
            size === 'xs' || size === 'sm' ? 'text-sm' : 'text-md'
          )}
        >
          {description}
        </p>
...
export const IconLeft = <T extends ComponentType>({
  ...
  return (
    <div
      className={cx(
        'group flex h-max flex-row items-start justify-start',
        size === 'xs' ? 'gap-2' : size === 'sm' ? 'gap-3' : 'gap-4'
      )}
    >
      ...
      <div
        className={cx(
          ...
          size === 'xs' || size === 'sm' ? 'pt-0.5 not-group-last:pb-6' : 'pt-1 not-group-last:pb-8'
        )}
      >
        <p
          className={cx(
...
export const IconTopNumber = <T extends ComponentType>({
  ...
  const statuses = {
    incomplete: {
      icon: 'bg-primary ring-1 ring-inset ring-secondary ',
      title: 'text-secondary',
      description: 'text-tertiary',
    },
    current: {
    ...
    >
      <div className={cx('relative flex w-full flex-col items-center self-stretch')}>
        ...
        {connector && (
          <span
            className={cx(
              'border-secondary absolute top-1/2 left-[53%] z-0 w-full flex-1 -translate-y-1/2 rounded-xs border-t-2',
      ...
      <div
        className={cx(
          'flex w-full flex-col items-start gap-0.5 self-stretch',
          status === 'incomplete' && 'opacity-60',
          (size === 'xs' || size === 'sm') && 'gap-0'
        )}
      >
        <p
          className={cx(
            'text-secondary w-full text-center',
            size === 'xs' || size === 'sm' ? 'text-sm font-semibold' : 'text-md font-semibold'
          )}
        >
          {title}
        ...
        <p
          className={cx(
            'text-tertiary w-full text-center',
            size === 'xs' || size === 'sm' ? 'text-sm' : 'text-md'
          )}
        >
          {description}
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center self-stretch">
        ...
        {connector && (
          <span
            className={cx(
              status === 'current' && 'opacity-60',
              'border-secondary flex-1 rounded-xs border-l-2',
              size === 'xs' || size === 'sm' ? 'my-1' : 'my-1.5'
            )}
          />
        )}
      ...
      >
        ...
        <p className={cx('text-tertiary', size === 'xs' || size === 'sm' ? 'text-sm' : 'text-md')}>
          {description}
        </p>
      </div>
    </div>
...
export const TextLine = <T extends ComponentType>({
  ...
  return (
    ...
    >
      <div
        className={cx(
          'bg-fg-brand-primary_alt absolute inset-x-0 top-0 h-1',
